...
char *getFasta::magic(int refId,int *posi,int numSites,perFasta *f){
 ...
 getFasta::getFasta(argStruct *arguments){
  ...
  anc=NULL;
   ...
   if(refName!=NULL){
    faiRef = fai_load(refName);
    if(faiRef==NULL){
      fprintf(stderr,"problems reading the (-ref ) reference genome\n");
      exit(0);
    }
   }
   if(ancName!=NULL){
    faiAnc = fai_load(ancName);
    if(faiAnc==NULL){
      fprintf(stderr,"problems reading the (-ans ) ancestral genome\n");
      exit(0);
    }
   }
}
