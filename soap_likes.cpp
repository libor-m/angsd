...
struct cmp_tmpStruct {
  bool operator()(const tmpStruct& first,const  tmpStruct& second) {
...
int calc_gl(chunkyT *chk,double *p_matrix,int which_sample,double **lk,int min_base_q,int trim) {
  int read_len = 256;
  //  fprintf(stderr,"[%s]\n",__FUNCTION__);
  ...
  for(int s=0;s<chk->nSites;s++) {
	...
	continue;
      ...
      if(nd.qs[j]<min_base_q||nd.posi[j]<trim||nd.isop[j]<trim|| refToInt[nd.seq[j]]==4 ){
	continue;
      }
...
void soap_likes::run(chunkyT *chk,double **lk,char *refs,int min_base_q,int trim) {
  assert(myMuts!=NULL);
  assert(chk!=NULL);

  if(doRecal==0){
    for(int i=0;i<chk->nSamples  ;i++)
      calc_gl(chk,p_matrix[i],i,lk, min_base_q,trim);
  }else{
    if(refs==NULL){
