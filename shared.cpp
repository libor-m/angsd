...
#define THRESHOLD_FOR_NICEOUTPUT 200 //we normally print whenever we change a chr, but don't bother if we have more the 200, looks ugly for contig data
...
void interface(funkyPars *p){
  //  if(p->killSig==1)
  //  fprintf(stderr,"chunkNumber=%d is the killSig, This is not error message.\n",p->chunkNumber);
...
funkyPars *allocFunkyPars(){
  ...
  r->results = new funkyPrint;
  r->results->freq = new funkyPrintFreq;
  r->results->asso = new funkyPrintAsso;
  //set nullpointers
  r->results->asso->stat=NULL;
  r->results->asso->keepInd=NULL;
  r->results->asso->freq=NULL;
  r->results->asso->lrt_snp=NULL;
  r->results->freq->pml=NULL;
  r->results->freq->pmlun= NULL;
  r->results->freq->pEM=NULL;
  r->results->freq->pEMun=NULL;
...
void deallocFunkyPars(funkyPars *p) {
  ...
  delete p->results->freq;
  delete p->results->asso;
  delete p->results;
...
//////////////////////////////////////////////////////////////////////////
//   Stuff to remove from this file
//////////////////////////////////////////////////////////////////////////
...
FILE *openFile(const char* a,const char* b,int i){
  char ary[5000];
  snprintf(ary,5000,"%s%d",b,i);
  return openFile(a,ary);  
}

void popBack(const char *a,const char*b,int to){
  char ary[5000];
  snprintf(ary,5000,"%s0-%d",b,to);
  dumpedFiles.push_back(strdup(ary));
}
